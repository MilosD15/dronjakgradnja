...
if (slidersWidth.indexOf("px") != -1) {
    ...
    var pom = slidersWidth.substring(0, pos);
    slidersWidth = parseInt(pom);
    slidingArea = slidersWidth;
}
else {
    slidersWidth = parseInt(slidersWidth.substring(0, slidersWidth.length - 1));
...
const electricianSliderContainer = document.querySelector("main .services .electrician_works .images .electrician_slider");
...
// controlling services section height
function ControlHeights() {
    var root = document.querySelector(":root");
    if(window.innerWidth <= 750) {
        var paint = getHeight("main .services .painting");
        var electrician = getHeight("main .services .electrician_works");
...
contactForm.addEventListener("submit", SendContactMessageListener);
...
openRes.addEventListener("click", () => {
    ...
    const arrow = document.querySelector("body > .ResWrapper > .con > .label > svg");
...
// closing 'choose language' con(container)
const langConCloseBtn = document.querySelector(".langWrapper .fa-times");
langConCloseBtn.addEventListener("click", () => {
    langCon.classList.toggle("active");
    contentCon.classList.toggle("opened");
    h1LangCon.classList.toggle("opened");
});
...
window.addEventListener("load", () => {
    ...
    if(document.documentElement.scrollTop > startHeight + headerHeight) {
        ...
        seeProjectsBtn.style.animation = "none";
    ...
    //     }
    // }
    // redirecting to the language appropriate page
    if (language == "en" && html_tag.lang != "en") location.href = "/en";
    if (language == "nl" && html_tag.lang != "nl") location.href = "/nl";
    if (language == "sr" && html_tag.lang != "sr") location.href = "/sr";
});

// images on hover always have the biggest z-index in about-us images mosaic grid
const aboutUsMosaicImages = document.querySelectorAll("main section.about-us .image .images-mosaic img");
aboutUsMosaicImages.forEach(img => {
    img.addEventListener("mouseenter", () => {
        aboutUsMosaicImages.forEach(img => img.style.zIndex = 1);
        img.style.zIndex = 2;
    });
});
aboutUsMosaicImages.forEach(img => {
